...
1. **제품/문서 등록**
   ...
   * 외부 OCR API 결과 텍스트를 서버에 저장하고 기본 필드 파싱 수행
   * 누락/불확실 시 **필드 추출 보완용 LLM**을 호출하여 보완, 이후 사용자 수동 수정
...
4. **알림 시스템 (이메일 + 텔레그램)**
   ...
   * 무료 PaaS에서는 서버 내부 스케줄러 대신 **외부 스케줄러(예: GitHub Actions)**가
     `POST /internal/cron/daily-alerts`를 호출하며, `X-CRON-SECRET`으로 보호됨
