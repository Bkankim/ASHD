...
1. **제품/문서 등록**
   ...
   * 업로드는 **202 + job_id**로 즉시 응답하고, OCR/추출은 **백그라운드 Job**으로 처리
   * 외부 OCR 결과 텍스트(raw_text)를 저장
   * 룰 기반 기본 필드 파싱(구매일/금액/구매처 등)
   * 누락/불확실 시 **필드 추출 보완용 LLM** 호출 + 사용자 수동 수정
   * 업로드 제한: 파일 최대 **10MB**, PDF는 **최대 3페이지**까지 처리(초과 페이지는 무시 + `job.error`에 경고 기록)
   * 무료 PaaS 특성상 업로드 파일은 best-effort 보관(소실 가능), 핵심 데이터는 DB에 저장된 `raw_text` 등 텍스트임
...
4. **알림 시스템 (이메일 + 텔레그램)**
   ...
   * 매일 1회: **외부 스케줄러(예: GitHub Actions)**가 `POST /internal/cron/daily-alerts` 호출
   * 헤더 `X-CRON-SECRET`(= `CRON_SECRET`)로 보호
   * 오늘 날짜 기준:

     * 보증 종료 D-30 / D-7 / D-3 등 임박 항목 조회
     * 환불 마감 D-3 임박 항목 조회
   * 이메일/텔레그램 요약 알림 발송(설정 없으면 안전하게 skip)
   * 상세 호출 예시는 `docs/DEV_GUIDE.md`, `docs/API_EXAMPLES.md` 참조
...
* RAG/LLM 기반 자연어 질의/요약 기능(assistant 형태)은 v0.1 제외
...
* 이후 기능 확장을 고려하여 다음을 염두에 둡니다.
  ...
  * LangGraph 기반 문서 처리 파이프라인을 `app/graphs/` 하위에 **v0.2 대비 설계**(현재 v0.1 런타임 필수는 아님)
...
* `AGENTS.md`
  ...
  * 의사결정/충돌 시 우선순위는 `AGENTS.md`의 `documents_priority`/`lookup_rule`
    (AGENTS.md > tasks/prd-*.md > tasks/tasks-prd-*.md > docs/* > code)을 따른다.
...
에이전트는 기능 설계/코드 변경 시,
먼저 이 `PROJECT_OVERVIEW.md`로 서비스 목적/범위를 이해한 뒤,
아키텍처/데이터/알림 세부는 각 문서에서 확인합니다. 상충 시 위 우선순위를 따릅니다.
